...
func TestXxx(t *testing.T) {
	...
	info, err := info{}.Map(int64(len(infoData)), *reader)
	assert.Nil(t, err)
	uid, _ := uuid.FromBytes([]byte{238, 6, 5, 118, 217, 31, 105, 95, 33, 117, 255, 128, 185, 32, 38, 185})

	expected := types.Info{
		SegmentUID:     uid,
		Duration:       594336,
		TimestampScale: 0xF4240,
		DateUTC:        1,
		WritingApp:     "mkvmerge v63.0.0 ('Everything') 64-bit",
